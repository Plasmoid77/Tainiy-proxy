...
- *Home machine behind NAT* — the LAN is already trusted, and with `deny incoming` SSH from the LAN would stop as soon as UFW came up. Flip the default and enable:

  ```bash
  ufw default allow incoming && ufw enable
  ```

  UFW then filters **only** `ygg0`: the script's `deny in on ygg0` plus the `--trusted` allows are the sole enforced rules, and `eth0`/Wi-Fi/LAN traffic passes as if there were no firewall. Re-running the script keeps working unchanged — it only prepends interface-bound rules and never touches the default policy. If the machine later gets a port forwarded from the internet, switch back with `ufw default deny incoming` *after* adding an SSH rule.

**Renaming the interface.** The rules are bound to the interface name, so `--iface` on a host that already runs the node would otherwise leave a dead `deny in on ygg0` and — worse — an `allow in on ygg0 from ADDR` that springs back to life if anything ever takes that name again. The script therefore reads the previous `IfName` from the live config before rewriting it and, when the name changes, deletes the rules it wrote for the old name — recognised by the interface and the exact comments `Yggdrasil: closed unless trusted` / `Yggdrasil trusted host` as listed by `ufw show added` — before prepending the new ones. Every trusted address found among the old rules is carried over to the new name, whether or not it was repeated as `--trusted` on that run, so a rename moves the access instead of dropping it. Rules you added yourself for the old name are not touched; neither is anything on a fresh install, where `IfName` is still `auto`. The script installs UFW if it is missing but never enables it: turning a firewall on without an SSH rule locks a remote session out. If UFW is inactive the rules are stored, a warning says so, and `ygg0` stays fully open until you allow SSH and run `ufw enable` (VPS-toolkit's `ufw-basic-setup.sh` does that part).
