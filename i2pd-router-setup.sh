...
SOCKS_PORT="${1:-9050}"
BACKUP_ROOT="/var/backups/deepwebproxy-backups/tor"
...
rollback_configuration() {
  ...
  restore_or_remove "torrc" "${TORRC}"
  restore_or_remove "tor@default.service.d" "/etc/systemd/system/${SERVICE_NAME}.d"
...
backup_if_exists "${TORRC}" "torrc"
backup_if_exists "${TOR_SOURCE_FILE}" "tor.sources"
backup_if_exists "${TOR_KEYRING}" "deb.torproject.org-keyring.gpg"
backup_if_exists "${TOR_PREFERENCES_FILE}" "torproject.pref"
backup_if_exists "/etc/systemd/system/${SERVICE_NAME}.d" "tor@default.service.d"
backup_if_exists "/etc/tor/torrc.d" "torrc.d"
...
IMPORTED_FINGERPRINT="$(
  gpg --batch --homedir "${TMP_DIR}/gnupg" --show-keys --with-colons "${TMP_DIR}/tor-signing-key.asc" \
...
Pin-Priority: 700
EOF_PREFERENCES
chmod 0644 "${TOR_PREFERENCES_FILE}"

log "Installing or updating Tor"
if ! apt-get update; then
  restore_or_remove "tor.sources" "${TOR_SOURCE_FILE}"
  restore_or_remove "deb.torproject.org-keyring.gpg" "${TOR_KEYRING}"
  restore_or_remove "torproject.pref" "${TOR_PREFERENCES_FILE}"
  apt-get update || true
  die "APT could not use the Tor Project repository; repository files were restored"
fi

TOR_APT_POLICY="$(apt-cache policy tor 2>/dev/null || true)"
if ! grep -Fq 'deb.torproject.org' <<< "${TOR_APT_POLICY}"; then
  restore_or_remove "tor.sources" "${TOR_SOURCE_FILE}"
  restore_or_remove "deb.torproject.org-keyring.gpg" "${TOR_KEYRING}"
  restore_or_remove "torproject.pref" "${TOR_PREFERENCES_FILE}"
  die "The Tor Project repository did not provide a package candidate for Debian 13"
fi
...
if [[ ! -e ${BACKUP_DIR}/torrc ]]; then
  cp -a "${TORRC}" "${BACKUP_DIR}/torrc"
fi

log "Overwriting Tor configuration with a local client-only configuration"
install -d -m 0755 "$(dirname "${TORRC}")"
cat > "${TORRC}" <<EOF_TORRC
# Managed by TainiyProxy.
# This server is a Tor client only. It is not a relay, bridge or exit node.
# The SOCKS5 listener is intentionally restricted to localhost.
...
if command -v ufw >/dev/null 2>&1; then
  if UFW_STATUS="$(ufw status verbose 2>/dev/null)" && grep -q '^Status: active' <<< "${UFW_STATUS}"; then
    ...
    if grep -Eq '^Default:.*(deny|reject) \(outgoing\)' <<< "${UFW_STATUS}"; then
      warn "UFW blocks outgoing traffic. Tor uses variable remote TCP ports and may not bootstrap."
...
for _ in $(seq 1 30); do
  TOR_JOURNAL="$(journalctl -u "${SERVICE_NAME}" -n 250 --no-pager 2>/dev/null || true)"
  if grep -q 'Bootstrapped 100%' <<< "${TOR_JOURNAL}"; then
...
printf '%b Mode: client only; relay, bridge and exit functions disabled%b\n' "${GREEN}" "${RESET}"
...
printf '%b Local test: curl --socks5-hostname %s:%s https://check.torproject.org/api/ip%b\n' \
  "${GREEN}" "${SOCKS_ADDRESS}" "${SOCKS_PORT}" "${RESET}"
printf '%b SSH tunnel: ssh -L %s:%s:%s USER@SERVER%b\n' \
